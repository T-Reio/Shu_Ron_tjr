...
Then, we consider the functional features that causes excess mass. In this paper, we avail the two models quated in Section 2: one is ``kink'' at the reference point, and the other is ``notch'' of the function.
...
Concretely, whether the players' benefit function has discontinuity in a certain performance index. Following Allen \textit{et al} (2016), we first use the assumption of the utility function of Diecidue and Van De Ven (2008)`s ``aspiration level" model. The model presents the form of the utility function with ``notch,''
