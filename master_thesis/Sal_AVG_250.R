...
e1 <- lm(Sal ~ BAT * AVG_250)
e2 <- lm(Sal ~ BAT * AVG_250 + FLD + BsR + Yr)
e3 <- lm(Sal ~ BAT * AVG_250 + FLD + BsR + AGE + AGE_sq + Yr)
e4 <- lm(Sal ~ BAT * AVG_250 + FLD + BsR + AGE + AGE_sq + WPA + nWPA + Yr)
e5 <- lm(Sal ~ BAT * AVG_250 + FLD + BsR + AGE + AGE_sq + WPA + nWPA + FA + Yr)
e6 <- felm(Sal ~ BAT * AVG_250 + FLD + BsR + AGE + AGE_sq + WPA + nWPA + FA + Yr | POS)
e7 <- felm(Sal ~ BAT * AVG_250 + FLD + BsR + WPA + nWPA + FA + Yr | ID)
e8 <- felm(Sal ~ BAT * AVG_250 + FLD + BsR + WPA + nWPA + FA + Yr| POS )

stargazer(e1,e2,e3,e4,e5,e6,e7,e8,
          out = 'C:/Users/easyu/Master_thesis/master_thesis/results/Sal_AVG250_B.tex',
          title = 'Replacing Batting-Average with BATTING, Including Interaction Term
          : around .250',
          align = F, initial.zero = F,
