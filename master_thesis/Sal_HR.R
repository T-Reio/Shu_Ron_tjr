...
e1 <- lm(Sal ~ BAT * HR_20)
e2 <- lm(Sal ~ BAT * HR_20 + FLD + BsR + Yr)
e3 <- lm(Sal ~ BAT * HR_20 + FLD + BsR + AGE + AGE_sq + Yr)
e4 <- lm(Sal ~ BAT * HR_20 + FLD + BsR + AGE + AGE_sq + WPA + nWPA + Yr)
e5 <- lm(Sal ~ BAT * HR_20 + FLD + BsR + AGE + AGE_sq + WPA + nWPA + FA + Yr)
e6 <- felm(Sal ~ BAT * HR_20 + FLD + BsR + AGE + AGE_sq + WPA + nWPA + FA + Yr | POS)
e7 <- felm(Sal ~ BAT * HR_20 + FLD + BsR + WPA + nWPA + FA + Yr | ID)
e8 <- felm(Sal ~ BAT * HR_20 + FLD + BsR + WPA + nWPA + FA + Yr| POS )

stargazer(e1,e2,e3,e4,e5,e6,e7,e8,
          out = 'C:/Users/easyu/Master_thesis/master_thesis/results/Sal_HR20_B.tex',
          title = 'Replacing Homerun with BATTING, Including Interaction Term
          : around 20 HR',
          align = F, initial.zero = F,
