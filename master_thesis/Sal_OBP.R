...
stargazer(d1,d2,d3,d4,d5,d6,d7,d8,
          ...
          intercept.bottom = FALSE, font.size = "tiny", label = "AVG350_A",
...
e1 <- lm(Sal ~ BAT * OBP_350)
e2 <- lm(Sal ~ BAT * OBP_350 + FLD + BsR + Yr)
e3 <- lm(Sal ~ BAT * OBP_350 + FLD + BsR + AGE + AGE_sq + Yr)
e4 <- lm(Sal ~ BAT * OBP_350 + FLD + BsR + AGE + AGE_sq + WPA + nWPA + Yr)
e5 <- lm(Sal ~ BAT * OBP_350 + FLD + BsR + AGE + AGE_sq + WPA + nWPA + FA + Yr)
e6 <- felm(Sal ~ BAT * OBP_350 + FLD + BsR + AGE + AGE_sq + WPA + nWPA + FA + Yr | POS)
e7 <- felm(Sal ~ BAT * OBP_350 + FLD + BsR + WPA + nWPA + FA + Yr | ID)
e8 <- felm(Sal ~ BAT * OBP_350 + FLD + BsR + WPA + nWPA + FA + Yr| POS )

stargazer(e1,e2,e3,e4,e5,e6,e7,e8,
          out = 'C:/Users/easyu/Master_thesis/master_thesis/results/Sal_OBP350_B.tex',
          title = 'Replacing On-Base Percentage with BATTING, Including Interaction Term
          : around .350',
          align = F, initial.zero = F,
          intercept.bottom = FALSE, font.size = "tiny", label = "AVG350_B",
          table.placement = "H", star.cutoffs = c(0.05, 0.01, 0.001),
          omit = c("WPA", "nWPA", "FA", "Yr"),
          add.lines = list(c("WPA", "","","","X","X","X","X","X"),
                           c("FA dummy","","","","","X","X","X","X"),
                           c("Season dummies","X","X","X","X","X","X","X","X"),
                           c("Fixed effects","","","","","",
                             "Position", "Individual","Position")
          )
)